...
class pP1D:
	...
	post_process = [
			...
			"qf":"[integration([1.0 - p for p in data['phi'][i]], data['vfx'][i], pF.dz/d_ad) for i in range(len(data['time']))]", # Only for wet cases
...
class pP2D:
	...
	post_process = [
			...
			{
			"fit":"np.polynomial.polynomial.polyval(bdata[bval]['A_fit'], np.polynomial.polynomial.polyfit(bdata[bval]['A'], bdata[bval]['phi_max'], 1))"
			},
			]
	add = [
			{
